...
function getInitialName() {
  //  retorna "Bruno"
  if (typeof window === "undefined") return "Bruno";
  return localStorage.getItem("nomeAtual") ?? "Bruno";
}

export default function AlterarNome() {
  const [currentName, setCurrentName] = useState(() => getInitialName());
  ...
  function handleConfirm() {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("nomeAtual", trimmed);
    }
  ...
  return (
    <div className="bg-[#7C3AED] min-h-screen flex flex-col items-center px-6">
      ...
      <div className="bg-white w-[95%] max-w-5xl mb-3 mt-3 rounded-3xl shadow-xl p-10 relative flex flex-col gap-8">
        ...
        <div>
          <Label className="ml-7 text-2xl font-bold mt-7 mb-0 block">
            Nome atual
          </Label>
        ...
        <form onSubmit={handleSubmit} className="w-full">
          <div>
            <Label className="ml-7 text-2xl font-bold mb-0 block">
              Alterar Nome
            </Label>
          ...
          <div className="flex flex-col items-center mt-6">
            <Button
              type="submit"
              className="ml-4 bg-[#7C3AED] hover:bg-[#752df1]  px-24 py-2 rounded-full text-white font-semibold justify-center shadow-none"
            >
              Confirmar
            </Button>
            {savedMessage && (
